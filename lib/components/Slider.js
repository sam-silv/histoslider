...
export default class Slider extends Component {
  ...
  mouseMove (e) {
    if (this.state.dragging) {
      ...
      selection = selection[0] > selection[1] ? [selection[1], selection[0]] : selection
      this.props.onChange(selection)
    }
  }
  ...
  render () {
    ...
    const selectionWidth = this.props.scale(selection[1]) - this.props.scale(selection[0])
    ...
    return (
      ...
      >
        <rect
          height={4}
          fill='#f1f1f1'
          x={ this.props.scale(selection[0]) }
          y={(innerHeight / 2) - 2}
